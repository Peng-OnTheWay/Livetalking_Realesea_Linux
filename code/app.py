...
def main():
    ...
    if opt.model == 'musetalk':
        ...
        global_avatars[opt.avatar_id] = load_avatar(opt.avatar_id)
        # v2(256)需要>=193输入，SAM(384)设计为192输入
        modelres = 256 if getattr(opt, 'wav2lip_model', '256') == '256' else 192
        opt.modelres = modelres  # 写回 opt，推理时使用
        warm_up(opt.batch_size, model, modelres)
